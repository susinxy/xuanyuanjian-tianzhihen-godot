...
- **S1 能力总评**：装配法典化/推进二态/事件总线/检查点自动注册四个设计决定全部
  ...
  **能力使用手册 = `docs/STAGE_ASSEMBLY.md` 第〇章食谱**（新地点=五件事+免费清单，
  最小样板 `scenes/stages/ref/stage_c.tscn`）
- **关账时点 backlog（审视产出，未立项）**：
  ① 互动/对话触发件缺失——S2 第一刀须补（装配九条+R10，别让胶水进场景）；
  ② S2 起"新机制契约套随交付同批"入验收定义（本轮教训：相机/车道全靠事后补锁）；
  ③ 检查点粒度=地点级（房内恢复/续谈归位待 S2/S3 裁决）；
  ④ 音频管道=零（S2/S3 至少埋 BGM 槽位）；⑤ 死亡壳无"重试当前房"（S3 议题）；
  ⑥ 全矩阵 23 套无分层执行脚本（低优先）；

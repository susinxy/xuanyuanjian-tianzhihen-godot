...
## 本脚本内嵌 24 套 / 25 跑名册（validator 双模=2 跑），与 AGENTS 收口一致；
...
##
## 退出码分区（终审波 FIX3：FATAL 全部 ≥64，与红套数 0-25 命名空间分离，
## CI 消费方可靠 rc 区分"判定=有红"和"环境=没跑成"）：
##   0-25 = 红套数（正常判定路径）；64 = 参数/环境错误；65 = destroy 先行失败；

...
## 引导时长（秒）：起手势（spell_start，角色资产）完整播完后，以保持姿势（spelling）
## 倒数本字段到零才释放法术体；总硬直=起手自然长+本值。0 = 无引导段，起手照播、
## 播完立即出手（2026-09-16 定档，废止单一计时器时代的"0=无动作瞬发"旧条款）。
...
### 17.0 弹体生命周期：系统淡入淡出契约（2026-09-16 定档，提交见本批）

- **形状**：`cast → 淡入(fade_in_time, α 0→1, 判定门=层 0) → ACTIVE(全亮, 门开) →
  命中/超时 → end()：有 ending 动画则美术信标接管，否则系统淡出(fade_out_time,
  α 渐熄, 门=0, 原地静止) → 0 后真删`。
- **时长是法术数据**：`SpellDefinition.fade_in_time / fade_out_time`，默认
  0.12 / 0.20（=全体法术出生自带）；填 0 显式关闭该阶段。
- **判定门=碰撞层全零**（不是只清高度位！）：场景默认 bit1(players 层) 有残留泄漏，
  受击盒掩码含基础位会照样发现——门关必须写 `collision_layer = 0`（逐帧 trace 定罪）。
  淡入期层恒 0 同时根治了"出生帧贴身秒杀"（公平性：首击不早于淡入完）。
- **驱动**：两个倒计时全在 `_physics_process`（物理帧时钟与移动/寿命同源；暂停
  冻结语义一致；headless 逐帧确定性）。alpha 与判定门读同一倒计时——测试断言
  「α<1 或已离场时层必须 0」即这条同源性的现场证明（禁用帧数窗口模拟：0.12s=7.2f
  的边界帧与固定窗口天然错位一帧，实机正确、断言假红）。
- **美术让位规则**：皮肤树声明 ending 动画 → 系统淡出不介入（防双重谢幕）；
  淡入无美术对应物，永远生效。淡入中途被击（α≈0）→ 淡出余量按当前亮度折算，
  瞬时销毁不闪跳。
- 防回归：`tools/spell_hit_test/` 19 断言（含双弹各自同呼吸、淡出残壳不补刀）。

### 17.1 施法者动画：两段式契约（2026-09-15 定稿，取代早期单段方案）

- **起手 `spell_start`**：非循环、**角色资产**（每角色一套、所有法术共享），
  自然时长、**任何法术都必须完整播完才进入引导段**——时长唯一信源=动画尾帧的
  `end_of_skin_animation` 方法调用（与攻击结束同款机制；auto_advance 方案经
  探针查证不可用：本项目非循环动画在动画树里引擎不自动宣告结束）。
- **引导 `spelling`**：循环（姿势保持），时长=法术定义 `caster_cast_time`
  （**只数引导段**；实际总硬直=起手自然长+本值；0=起手完立即出手，起手恒必播）。
  循环天然适配任意引导时长——**禁止变速/掐断/定格**中间态。
- 两槽均为四点 BlendSpace2D（结构照抄 attack1，四输入暂各指单一动画），
  将来分方向只换输入节点动画名，接线零动。
- 占位资产（attack1_right 派生）：`spell_start.tres` 保留 end_of_skin_animation、
  删 end_of_input_frames 与**全部 Attacks 盒轨道**（不删则施法顺手打中人）、非循环；
  `spelling.tres` 删全部方法轨+线性循环。
- 降级阶梯（缺资产时计时一律不变）：缺起手→跳段直接上膛引导；缺循环→起手定格
  保持；全缺→切 idle（有则）+一次性告警。
- **美术真帧约定**：起手姿势画左右中性（正面抱球/双手合印），单动画才能在任意
  朝向成立；**起手尾帧姿势=引导循环首帧姿势**，衔接由帧保证。
- **信标通道决议（2026-09-15 复核维持）**：起手结束沿用通用总线 `end_of_skin_animation`（专用信号方案评估后否决）——QuiverState enter/exit 自动挂/摘监听 + 同一时刻仅一个叶子动作状态存活 = 结构性单听众保证。除非未来出现「同一状态需区分多段动画结束」（如三段化加收势），勿再改议。
- **落地状态**：chen 两段真帧美术已完成并经用户 F5 体验验收，模板已 sync（新角色出生即带两槽与接线）；fire_ball 弹体美术走母版/缩放管线（同日入库）。
- **信标的适用范围（2026-09-16 定档）**：`end_of_spell_animation` 只属于**一次性**
...
- **多实例隔离（实测坐实，非推定）**：法术信标/通知每一环都是实例私有——每次
  ...
  双弹同屏实测：两次掉血帧 [23,36] 与两次消亡帧 [24,37] 严格一对一配对，零串线
  （tools/spell_hit_test/projectile_hit.gd 永久断言）。

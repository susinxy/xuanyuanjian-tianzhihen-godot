...
- 白闪共享小函数（约 10 行，出处定档一处）：对角色皮肤 `modulate` 短回弹（弹反 ≈0.12s 亮档、格挡 ≈0.07s 微档；时长/色为规则常量单一出处）。现件无生产命中 VFX 先例（legacy 台本有 modulate 回弹原型可参照）。
  ...
  - **二次修订（同日 F5 手感反馈"短且生硬"）**：线性升 40%/降 60% 形制废弃，改**瞬白峰→持帧→SINE/EASE_OUT 缓落**——弹反 0.06s 持帧+0.22s 落、格挡 0.03s+0.14s；**同日三调（"再长一点"）定档弹反 0.08s+0.30s、格挡 0.04s+0.18s**（常量 `_FLASH_*_HOLD/_FADE` 单一出处，契约摘净窗格挡 ≤30/弹反 ≤40 帧=残留哨兵窗）；实测弹反的 6 帧定格期间白闪同步冻存，全白段被定格拉长（三调后防守方末见 ≈28 帧 vs 旧 7 帧）。回归锁=block_parry_contract P2g/P2h/P3j-P3l。
...
- 弹反的**双方视觉响应三件套**（用户 2026-09-23 补案）：防守方爆闪（强档）+ **攻击者同拍微白闪**（复用同一 helper 打在攻击者皮肤上）+ 加强定格让被顶的一下被看清——主体表达始终是攻击者自己的受击动画，零新增资产。
  - **定格 6 帧裁决（2026-09-24 F5 关账）**：用户实测"顶硬（他挨自己一下+退步）成立，但 6 帧≈0.1s 停顿在无声学件下处于感知阈值边缘、主观不易察觉"（普攻底数 3 帧，弹反仅多咬 0.05s）。**裁决=暂保持 6 帧**，不加到 9~12（白闪时长已验收"刚好"，再拉长会过头）；弹反的进一步强化留给 B6 音效位（铛一声才是这 6 帧的真正载体），**在此之前勿把"定格无感"当 bug 回炉此常量**。

...
### 眼③ 死亡段重跑：仍会、秘籍**不复活**（外观判重，2026-09-25 用户裁决改判）

故意站桩让小练打死：

| 预期 | 实际 | 过不过 |
|---|---|---|
| 死亡→段重跑（D4，落回出生位）后按 1 **仍会**（出生补学块查账回填）；重跑=未判清段丢弃重建，但**秘籍不再出现**——账本有账时本件出生帧连触发件整体退场（一次性知识件不重弹"拾取秘籍"，chest 空箱物理语义不适用于书）。本眼首跑曾按旧口径报"书复活不合理"，已改判入生产（契约 G6a/b 双腿锁死，含无账负对照） |  |  |

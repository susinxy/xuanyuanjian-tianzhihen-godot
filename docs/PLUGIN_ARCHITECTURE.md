...
12. 统一轨道注入（`_inject_all_tracks()`）
    - **Attack 动画结构体检**（`_validate_attack_animation_structure()`，仅 attack 类）：缺 `end_of_skin_animation` 方法轨道（攻击态无出口；`air_*` 豁免——空中出口由 `_end_condition` 决定）或所有 `:disabled` 恒 true（攻击盒永不激活）→ 写入 errors 大声报障（不自动修——时刻/窗口属手感设计）。背景：模板曾长期带病发布 `attack*_down/up`（缺全部机制轨道，致 2026-09-12 起照抄 right 结构治愈 18 文件），此体检防任何来源复发

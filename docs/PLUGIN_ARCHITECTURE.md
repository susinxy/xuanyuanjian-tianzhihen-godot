...
**皮肤方向契约（2026-09-14 补全）**：`QuiverCharacterSkin.skin_direction` 的语义是
**任意世界方向向量**（任意角度/幅度）。八向混合空间是顶点在 45°k 的正八边形，
凸包边外是权重塌陷区（表现为角色隐形/幽灵，阴影随动闪烁）——旧实现依赖"输入恰好
只在八个顶点方向"的巧合，AI 连续角度（及任何手柄模拟量）会掉进洞。修法为**几何本征
映射**而非防御钳制：setter 调用 `project_to_blend_domain()` 做精确径向投影
（`r(α)=cos22.5°/cos(22.5°−α)`，α=与最近顶点夹角；域内恒等、域外沿射线落边、角度
严格保持）。玩家离散/归一化输入全部位于域内 → 对既有手感数学恒等（性质测试含八点
恒等断言）。**任何未来新输入源（弹道、路径跟随）无需自识几何**。

**回归验证**：`tools/input_channel_test/test_runner.tscn`（headless 17 断言：通道单元
语义、玩家移动/攻击链路、被动角色不被物理键盘劫持、窗口门控、AI 注入链路）与
`tools/blend_domain_test/project_test.gd`（16 项几何性质断言：顶点恒等/360° 角保持
且落域/幂等/角落与边中点精确值）。
